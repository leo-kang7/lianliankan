...
void GameModel::startGame(GameLevel level)
{
    ...
    gameMap = (int *)malloc(sizeof(int) * MAX_ROW * MAX_COL);   //分配空间
    memset(gameMap, 0, MAX_ROW * MAX_COL);
    //memset易出错
    for (int i = 0; i < MAX_ROW * MAX_COL; i++)
        gameMap[i] = 0;

    hintArray = (int *)malloc(sizeof(int) * 4);                 //分配空间
...
bool GameModel::isFrozen()
{
    ...
    for (int i = 0; i < MAX_ROW * MAX_COL - 1; i++)
        ...
        {
            int srcX = i % MAX_COL;     // 第一个点的x坐标
            int srcY = i / MAX_COL;     // 第一个点的y坐标

            int dstX = j % MAX_COL;     // 第二个点的x坐标
            int dstY = j / MAX_COL;     // 第二个点的y坐标
